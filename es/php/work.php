...
function sendEmail($name,$email,$phone, $address, $col, $city, $skill, $mailto, $mailto2){
...
function validate($name,$email, $phone, $address, $col, $city, $skill, $mailto, $mailto2){
  if(!isset($name) || $name==='' || !isset($email) || $email==='' || !isset($phone) || $phone==='' || !isset($address) || $address==='' || !isset($col) || $col==='' || !isset($city) || $city==='' || !isset($skill) || $skill===''){
    response('Por favor complete todos los campos, </br> vuelva a intentarlo',false,401,"campos imcompletos");
  }else{
    sendEmail($email,$name,$phone, $address, $col, $city, $skill, $mailto, $mailto2);
  }
}

validate($email,$name,$phone, $address, $col, $city, $skill, $mailto, $mailto2);
